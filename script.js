...
function fetchBooks() {
  fetch(endpoint)
    ...
    .then((data) => {
      ...
      } /* else if (inputSearch.length === 1 || inputSearch.length === 2) {
        return;
      } */ else {
        showBooks(data);
      }
...
function showBooks(books) {
  ...
  books.forEach((book) => {
    ...
    cardLikeIcon.classList.add("bi", "bi-heart");
    cardLikeIcon.style.color = "#920202";
    const cardCartIcon = document.createElement("i");
    cardCartIcon.classList.add("bi", "bi-cart");
    cardFooterDiv.append(cardLikeIcon, cardCartIcon);
